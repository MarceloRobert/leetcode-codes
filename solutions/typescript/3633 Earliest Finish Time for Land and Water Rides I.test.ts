...
function earliestFinishTime(landStartTime: number[], landDuration: number[], waterStartTime: number[], waterDuration: number[]): number {
    ...
    // maybe work with intervals? either way it would depend on hwo the solution is designed
    // we could do a "for each ride of type A, check each ride of type B and store the earliest finish time comparing between the pairs"
    // maybe that will be too slow? O(n*m)

    let earliestFinish = Infinity;

    // Check land -> water rides
    for (let i = 0; i < landStartTime.length; i++) {
        const landFinish = landStartTime[i] + landDuration[i];
        for (let j = 0; j < waterStartTime.length; j++) {
            const mismatch = waterStartTime[j] - landFinish;
            let waterFinish = 0;
            if (mismatch > 0) {
            ...
            if (waterFinish < earliestFinish) {
                earliestFinish = waterFinish;
            }
        }
    }

    // Check water -> land rides
    for (let j = 0; j < waterStartTime.length; j++) {
        const waterFinish = waterStartTime[j] + waterDuration[j];
        for (let i = 0; i < landStartTime.length; i++) {
            const mismatch = landStartTime[i] - waterFinish;
            let landFinish = 0;
            if (mismatch > 0) {
                landFinish = landStartTime[i] + landDuration[i];
            } else {
                landFinish = waterFinish + landDuration[i];
            }

            if (landFinish < earliestFinish) {
                earliestFinish = landFinish;
            }
        }
    }
